...
class SarifOutputReport : OutputReport() {
    ...
    override fun render(detektion: Detektion): String {
        val descriptors = ruleDescriptors(config)
        val report = detektSarif {
            for ((ruleSetId, issues) in detektion.findings) {
                for (issue in issues) {
                    results.add(issue.toIssue(ruleSetId, descriptors))
                }
            }
        }
...
fun Finding.toIssue(ruleSetId: RuleSetId, descriptors: HashMap<String, ReportingDescriptor>): SarifIssue = result {
    ruleId = "$ruleSetId>$id"
    val ruleDescriptor = descriptors[ruleId]
    if (ruleDescriptor != null) {
        rule = ReportingDescriptorReference().apply {
            id = ruleDescriptor.id
            guid = ruleDescriptor.guid
            toolComponent = DetektToolReference
        }
    }
